...
<body background="${pageContext.request.contextPath}/slike/slika8.jpg">
	...
	<div class="divUnos">
		...
		<c:if test = "${!empty naocareIzabraneKat}">
			...
			<table>
					...
					<c:forEach items = "${naocareIzabraneKat}" var = "n">
				
						<tr>
							<td> ${n.naziv} </td>
							<td> ${n.kategorija.nazivKategorije} </td>
							<td> ${n.cena} </td>
							<td> <a href = "/NaocareShop/korisnici/prikazSlikeZaKat?idNaocara=${n.idnaocare}"> Prikazi sliku </a> </td>
						</tr>
					</c:forEach>
			</table>
		</c:if>	
